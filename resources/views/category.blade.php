  ...
  <div class="md:grid md:grid-cols-12 md:gap-12">
    <div class="md:col-span-8 2xl:col-span-9">
      <div>
        ...
        <p class="text-gray-500 mb-10">{{ $category->description }}</p>
        @if (!$posts->isEmpty())
          <div class="md:grid md:grid-cols-2 gap-6 space-y-4 md:space-y-0">
            @foreach ($posts as $post)
              <x-post.card :post="$post" class="!bg-gray-50 !pt-3.5 !pb-5 !px-5 !text-lg !border-t-2 !border-t-green-500" />
            @endforeach
          </div>
        @else
          <div class="bg-gray-50/50 rounded-lg p-24 flex items-center justify-center">
    ...
    <div class="md:col-span-4 2xl:col-span-3 space-y-12">
      <div>
        ...
        <form action="{{route('blog')}}" class="flex items-center space-x-4">
          <x-text-input class="!py-1.5" name="filter[q]" type="text" />
